...
void COXLayoutManager::CalcOffsets( COXWndConstraint* pWC)
{
	CRect childRect;
	CWnd* pChildWnd = m_pContainerWnd->GetDlgItem(pWC->nID);
	pChildWnd->GetWindowRect(childRect);
	if(pWC->sc[0].IsEmpty())
		SetConstraint(pChildWnd, OX_LMS_TOP, OX_LMT_SAME, childRect.top);
	if(pWC->sc[1].IsEmpty())
		SetConstraint(pChildWnd, OX_LMS_LEFT, OX_LMT_SAME, childRect.left);
	m_pContainerWnd->ScreenToClient(childRect);
	for (int j = 0; j < 4; j++)
	{
		COXSideConstraint* pSC = &pWC->sc[j];
		
		int nSingleSide = OX_INDEX_TO_LMS[j];
		int nBasePos1 = OX_LMPOS_NULL;
		int nBasePos2 = OX_LMPOS_NULL;
		BOOL bVert = j > OX_LMSI_HORZ;
		if (pSC->nBaseWnd == 0)
		{
			nBasePos1 = 0;
			nBasePos2 = (bVert ? m_cx : m_cy);
		}else{
			CWnd* pWnd = m_pContainerWnd->GetDlgItem(pSC->nBaseWnd);
			ASSERT(pWnd);
			if (pWnd == NULL) continue;
			CRect rect;
			pWnd->GetWindowRect(rect);
			m_pContainerWnd->ScreenToClient(rect);
			nBasePos1 = (bVert ? rect.left : rect.top);
			nBasePos2 = (bVert ? rect.right : rect.bottom);
		}
		
		if (!pSC->IsEmpty())
		{
			int off1=OX_LMPOS_NULL;
			int off2=OX_LMPOS_NULL;
			int nType =0;
			if (pSC->nOffset1 == OX_LMOFFSET_ANY)
			{
				if (nSingleSide & OX_LMS_MINOR)
					off2 = (bVert ? childRect.right : childRect.bottom)-nBasePos2;
				else
					off2 = (bVert ? childRect.left : childRect.top)-nBasePos2;
				pSC->nPos=off2;
			}else if (pSC->nOffset2 == OX_LMOFFSET_ANY)
			{
				if (nSingleSide & OX_LMS_MAJOR)
					off1 = (bVert ? childRect.left : childRect.top)-nBasePos1;
				else
					off1 = (bVert ? childRect.right : childRect.bottom)-nBasePos1;
				pSC->nPos=off1;
			}else
			{
				if (nSingleSide & OX_LMS_MAJOR)
					off1 = (bVert ? childRect.left : childRect.top)+2;
				else
					off1 = (bVert ? childRect.right : childRect.bottom)+2;
				pSC->nPos=off1;
				off1 = (int)(off1*m_nBase/nBasePos2);
				off2 = m_nBase;
			}
			pSC->nOffset1=off1;
			pSC->nOffset2=off2;
...
void COXLayoutManager::Attach(CWnd* pContainerWnd)
{
	...
	CRect containerRect;
	m_pContainerWnd->GetClientRect(containerRect);
	m_cx=containerRect.Width();
	m_cy=containerRect.Height();
	for (int i = 0; i < m_wcTable.GetSize(); i++)
	{
		CalcOffsets(m_wcTable[i]);
	}
	OnSize(m_cx, m_cy);
}
...
int COXLayoutManager::CalcLayout()
{
	...
	{
		COXWndConstraint* pWC = m_wcTable[i];
		if (pWC != m_pTrackingWnd)
		{
			pWC->ResetPos();
		}else{
			for (int i=0;i<4;i++)
			{
				COXSideConstraint* pSC=pWC->sc+i;
				if (!pSC->IsEmpty() && pSC!=m_pTrackingSide) pSC->nPos=OX_LMPOS_NULL;
			}
		}
	}
...
{
	int i = 0;
	for (i = m_wcTable.GetSize() - 1; i >= 0 && m_wcTable[i]->nID != nChildWnd; i--);
	return i;
}
	...
	// --- Effect : This function subclasses the container window 
{
	...
	if (m_pfnSuper != NULL)
	{
		...
		if ( (m_hWnd != pContainerWnd->GetSafeHwnd()) || 
		     ((WNDPROC)(LONG)::GetWindowLong(pContainerWnd->GetSafeHwnd(), GWL_WNDPROC) != GlobalLayoutManagerProc) )
		{
	...
	m_pfnSuper = (WNDPROC)(LONG)::GetWindowLong(pContainerWnd->GetSafeHwnd(), GWL_WNDPROC);
	...
	//				COXLayoutManager object
{
	...
	switch (uMsg)
	{
		...
		case WM_LBUTTONDOWN:
			xPos = GET_X_LPARAM(lParam); 
			yPos = GET_Y_LPARAM(lParam);
			OnLButtonDown(wParam, xPos, yPos);
			break;
		case WM_LBUTTONUP:
			xPos = GET_X_LPARAM(lParam); 
			yPos = GET_Y_LPARAM(lParam);
			OnLButtonUp(wParam, xPos, yPos);
			break;
		default:
			// Do nothing special
			;
...
void COXLayoutManager::OnMouseMove( int flags, int x, int y )
{
	x=__min(__max(0,x),m_cx);
	y=__min(__max(0,y),m_cy);
	if (m_pTrackingSide)
	{
		if (flags & MK_LBUTTON)
		{
...
void COXLayoutManager::SetCursor( int x, int y )
{
	int i,nSide;
	for (i=0; i<m_wcTable.GetSize(); i++)
	{
		...
		{
			int nSingleSide = OX_INDEX_TO_LMS[nSide];
			COXSideConstraint* pSC = &pWC->sc[nSide];
			int& d1=pSC->nOffset1;
			int& d2=pSC->nOffset2;
			if ((d1!=OX_LMOFFSET_ANY) && (d2!=OX_LMOFFSET_ANY))
			{
				BOOL bVert = nSide > OX_LMSI_HORZ;
				int mPos=(bVert ? y:x);
				CRect rect=pWC->GetRect();
				int mPosMin=(bVert ? rect.top:rect.left);
				int mPosMax=(bVert ? rect.bottom:rect.right);
				int offset=(bVert ? x:y)+m_nSplitterSize-(int)(d1*(bVert ? m_cx : m_cy)/d2);
				if ((abs(offset)<m_nSplitterSize) && (mPos>=mPosMin) && (mPos<=mPosMax))
				{
					::SetCursor(bVert ? hSizeWE : hSizeNS);
					return;
				}
			}
		}
...
void COXLayoutManager::Tracking( int x, int y )
{
	CRect rect = m_pTrackingWnd->GetRect();
	CWnd* pControl = m_pContainerWnd->GetDlgItem(m_pTrackingWnd->nID);
// 	pControl->GetWindowRect(&rect);
// 	m_pContainerWnd->ScreenToClient(&rect);
	long& rside=rect.left;
	int i;
	for (i=0;i<4;i++)
		if (m_pTrackingWnd->sc+i == m_pTrackingSide)
			break;
	BOOL bHoriz= ( i>OX_LMSI_HORZ ? TRUE : FALSE);
	switch (i)
	{
	case OX_LMSI_TOP:
		rside=rect.top;
	...
	int newPos= (bHoriz ? x : y);
	if (rside != newPos)
	{
		rside=newPos;
		m_pTrackingSide->nPos=newPos;
		pControl->MoveWindow(&rect,TRUE);
		OnSize(m_cx,m_cy);
		pControl->GetWindowRect(&rect);
		m_pContainerWnd->ScreenToClient(&rect);
	}
...
void COXLayoutManager::StopTracking()
{
	m_pTrackingSide->nOffset1=0;
	m_pTrackingSide->nOffset2=m_nBase;
	CalcOffsets(m_pTrackingWnd);
	m_pTrackingSide=NULL;
	m_pTrackingWnd=NULL;
	m_nOffset=0;
	::ReleaseCapture();
...
void COXLayoutManager::OnLButtonDown( int flags, int x, int y )
{
	int i,nSide;
	for (i=0; i<m_wcTable.GetSize(); i++)
	{
		...
		for (nSide = 0; nSide < 4; nSide++)
		{
			...
			if ((d1!=OX_LMOFFSET_ANY) && (d2!=OX_LMOFFSET_ANY))
			{
				...
				if ((abs(offset)<m_nSplitterSize) && (mPos>=mPosMin) && (mPos<=mPosMax))
				{
					m_nOffset=offset;
					m_pTrackingSide = pSC;
					pSC->nOffset2=OX_LMOFFSET_ANY;
					m_pTrackingWnd = pWC;
					::SetCursor(bVert ? hSizeWE : hSizeNS);
					m_pContainerWnd->SetCapture();
					return;
				}
			}
		}
...
void COXLayoutManager::OnLButtonUp( int flags, int x, int y )
{
	if (m_pTrackingSide)
	{
		StopTracking();
...
COXSideConstraint* COXLayoutManager::GetMovableSide( int x, int y, int* nSide )
{
	return NULL;
}
/////////////////////////////////////////////////////////////////////////////
